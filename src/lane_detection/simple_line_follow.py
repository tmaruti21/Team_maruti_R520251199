...
class LaneDetectorNode(Node):
    ...
    def image_callback(self, msg):
            ...
            if len(slopePositiveLines) > 0 and len(slopeNegativeLines) > 0:
                ...
                if len(xInterceptPos) > 0:
                    ...
                    if len(xInterceptNeg) > 0:
                        ...
                        #-------------------------------------NEAR LOOK-AHEAD NAVIGATION-----------------------------------------
                        # Evaluate the lane centre at a SHORT distance ahead of the rover
                        # (look_ahead_fraction of frame height, in flipped-y coords where
                        # y=0 is the image bottom = closest to rover).
                        # This means the deviation only grows when the turn is RIGHT HERE,
                        # not when it is still visible but far away.
                        ...
                        # Normalised lateral error: +1 = lane is fully right, -1 = fully left
                        error = (lane_center_ahead - frame_center_f) / frame_center_f
                        error = max(-1.0, min(1.0, error))

                        # --- Velocity command ---
                        # Slow down proportional to how much we need to steer.
                        # Right error → negative angular.z (ROS: CCW positive)
                        twist = Twist()
                        ...
                        # --- Update turn state for display overlay ---
                        ...
                        if self.turn_detected and self.turn_direction:
                            ...
                            # Log turn detection
                            if self.frameNum % 30 == 0:
                                self.get_logger().info(f'↩ TURNING {self.turn_direction}  error={error:+.2f}  ang={twist.angular.z:+.2f}')
                        
                        # Display navigation info
                        nav_info = (f"Err:{error:+.2f}  "
        ...
        # If no lanes detected this frame, stop the rover
...
if __name__ == '__main__':
    main()
# this will follow a simple straight line